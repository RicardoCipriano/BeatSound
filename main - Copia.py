...
class BeatSoundSearch(ctk.CTk):
    def __init__(self):
        ...
        # Define geometria ocupando toda a tela
        self.geometry(f"{screen_width}x{screen_height}+0+0")

        # FORÇA a janela a aplicar a geometria imediatamente
        self.update_idletasks()

        # Maximiza a janela (garante que tudo fique visível)
        self.state('zoomed')
    ...
    def _delayed_setup(self):
        ...
        try:
            ...
            file_btn = self.menu.add_cascade("File", fg_color="#c3000d")
            file_menu = CustomDropdownMenu(widget=file_btn)
            file_menu.add_option(option="Sincronizar Biblioteca", command=lambda: self.navigate_to("home"))
            file_menu.add_option(option="Sair", command=self.quit)
            
            edit_btn = self.menu.add_cascade("Edit", fg_color="#c3000d")
            edit_menu = CustomDropdownMenu(widget=edit_btn)
            edit_menu.add_option(option="Configurações", command=lambda: self.navigate_to("settings"))
            
            view_btn = self.menu.add_cascade("View", fg_color="#c3000d")
            view_menu = CustomDropdownMenu(widget=view_btn)
            view_menu.add_option(option="Principal", command=lambda: self.navigate_to("home"))
            view_menu.add_option(option="Modo Escuro", command=lambda: self.set_theme("dark"))
            view_menu.add_option(option="Modo Claro", command=lambda: self.set_theme("light"))
            
            play_btn = self.menu.add_cascade("Player", fg_color="#c3000d")
            play_menu = CustomDropdownMenu(widget=play_btn)
            play_menu.add_option(option="Play/Pause", command=self.toggle_play_pause)
            play_menu.add_option(option="Next", command=self.play_next)
            play_menu.add_option(option="Prev", command=self.play_prev)
            ...
            for m in ["File", "Edit", "View", "Player"]:
                ctk.CTkLabel(self.menu_frame, text=m, font=("Segoe UI", 12)).pack(side="left", padx=10)
    ...
    def setup_ui(self):
        ...
        # View container to act as body above player
        ...
        ctk.CTkLabel(self.sidebar, text="Ferramentas", font=("Segoe UI", 12), text_color=TEXT_MUTED).pack(anchor="w", padx=20, pady=(20, 5))
        ctk.CTkButton(
            self.sidebar, text="✏️ Editor Tags", anchor="w", fg_color="transparent",
            text_color=TEXT_COLOR, hover_color="#2b2b2b", font=("Segoe UI", 14),
            command=lambda: self.navigate_to("editor")
        ).pack(pady=5, padx=10, fill="x")

        ctk.CTkButton(
            self.sidebar, text="📦 Álbum Único (Lote)", anchor="w", fg_color="transparent",
            text_color=TEXT_COLOR, hover_color="#2b2b2b", font=("Segoe UI", 14),
            command=lambda: self.navigate_to("batch", data={"mode": "single"})
        ).pack(pady=5, padx=10, fill="x")

        ctk.CTkButton(
            self.sidebar, text="🏛 Gestor Biblioteca", anchor="w", fg_color="transparent",
            text_color=TEXT_COLOR, hover_color="#2b2b2b", font=("Segoe UI", 14),
            command=lambda: self.navigate_to("manager")
        ).pack(pady=5, padx=10, fill="x")

        ctk.CTkButton(
            self.sidebar, text="🤖 Lote Variável (IA)", anchor="w", fg_color="transparent",
            text_color=TEXT_COLOR, hover_color="#2b2b2b", font=("Segoe UI", 14),
            command=lambda: self.navigate_to("batch", data={"mode": "variable"})
        ).pack(pady=5, padx=10, fill="x")

        ctk.CTkButton(
            self.sidebar, text="🚀 Download Music", anchor="w", fg_color="transparent",
            text_color="#2ecc71", hover_color="#2b2b2b", font=("Segoe UI", 14, "bold"),
        ...
        # self.sidebar_handle.place(relx=0.99, rely=0.4, anchor="center")
    ...
    def setup_player_bar(self):
        ...
        # Seeker Slider (instead of static progress bar)
        self.seeker = ctk.CTkSlider(self.player_bar, height=15, progress_color=RED_ACCENT, fg_color="#333333",
    ...
    def toggle_shuffle(self):
        ...
        # Visual feedback: changing both color and a small bulb indicator if possible, or just clearer color
        color = "#2ecc71" if self.player.shuffle else "#b3b3b3" # Green for enabled
    ...
    def toggle_repeat(self):
        ...
        modes = ["🔁 Off", "🔂 One", "🔁 All"]
        # Cycle: Off (gray) -> One (green) -> All (green)
